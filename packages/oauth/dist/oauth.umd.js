(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["oauth"] = factory(require("vue"));
	else
		root["oauth"] = factory(root["Vue"]);
...
;// CONCATENATED MODULE: ./src/oauth.js
/**
...
const handleOAuth = ({
  ...
  instance = new (external_commonjs_vue_commonjs2_vue_root_Vue_default())({
    data() {
      return {
        token: undefined,
        user: undefined
    ...
    methods: {
      ...
      async fetchToken(code, state, provider) {
        ...
        const client = await lib_axios.create({
          baseURL: options.sso,
          headers: {
            "Content-Type": "application/json"
          }
        });
        const tokenResponse = await client.get(`/oauth2/access_token?code=${code}&provider=${provider}`, {}).then(response => response.data).catch(error => {
          onFailure(error);
        });
        ...
        // ID Token
        let idToken = jwt_decode_esm(tokenResponse['id_token']);
    ...
    async created() {
      try {
        ...
        if (params.get('code') && params.get('state') && params.get('provider')) {
          await this.fetchToken(params.get('code'), params.get('state'), params.get('provider'));
...
/* harmony default export */ const index = (oauth);
...
//# sourceMappingURL=oauth.umd.js.map